...
Optimization部分因为结合了很多的g2o代码，所以读起来会相对比较困难，因此这里整理了[口哨糖youri](https://blog.csdn.net/qq_47891062)的一些整理的笔记，对Opimizer再做一遍梳理，也提升一下自己对于g2o的理解。

### 单帧优化：
- Optimizer::PoseOptimization(Frame *pFrame)
  - 这一个相对比较简单，就是将三维的地图点重投影到图像上，都是一元边。另外在流程上就是创建优化器，初始化点和边，然后利用卡方分布的阈值来剔除外点。每一轮的外点不参与优化，但是这一轮的外点下一轮依旧会参与计算，依旧可能成为内点
- Optimizer::PoseInertialOptimizationLastFrame(Frame *pFrame, bool bRecInit)
  - 这个是在出现Fail to track时出现比较多的，因此着重看一下。
  - 在该里面有三个约束，一个是和之前的相同的重投影（视觉约束），对于IMU的约束则是希望IMU的预积分与两帧之间的实际速度变化相同，最后则是因为设置了上一帧的也会加入优化，所以需要对陀螺仪的两个加速度的变化量加以惩罚。另外会添加希望上一帧信息改变的量要尽可能小。
  - 在优化时，会参考地图点的深度。如果深度小于某一个阈值会设置其为近点（因为近大远小？）。有三种情况会被设置为外点：深度值为负，近点小于一个偏大的阈值，远点小于一个小一些的
  - 如果太烂的话会往里面补一些内点（会不会是因为补进去的内点太差劲了？）
  - 同时保存当前帧的各种估计值和被边缘化的H矩阵，用于下一帧使用的EdgePriorPoseImu
-  Optimizer::PoseInertialOptimizationLastKeyFrame(Frame *pFrame, bool bRecInit)：这个函数与Fframe只是在数据格式上有些差别，在其他方面是一模一样的。
### 全局优化和局部地图优化
基本上与单帧的优化是一致的，只不过需要确定哪些点的位姿是不变的，从而Fix住一些G2O的优化边，同时会加入比较多的边

## 闭环检测
1. 首先在有新的关键帧被检测到的时候触发，然后判断两个关键帧之间是否真的有共视区域。判断共视区域的标准为IMU是否已经初始化，关键帧数量是否比较少。
2. 之后根据BoW来选择待定的有共视区域的帧，先挑有相同单词（特征点可以被聚类成同一个单词，类比于特征向量的区别小于一个阈值？）然后将有相同单词最多的点乘上0.8来作为下限来筛选（有点类似于在选匹配的时候distance限制）筛选之后再每个关键帧跳出来10个共视帧作为一组，再计算一组的相似分数，通过按照分数降序排列来得到候选的N个关键帧
3. 得到候选之后来逐个计算如果要做回环的话需要的位姿，在该位姿上能匹配上，匹配后的内点数量满足要求，并且最终的位姿依旧能够有足够的匹配点，这样才能够认为是有了共视的区域
